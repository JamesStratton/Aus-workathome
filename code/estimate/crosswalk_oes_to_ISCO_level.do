...
//Assert that wage is constant in each four-digit ISCO x level 
egen isco_mean_wage = mean(wage), by(ISCO08_Code level)

* ------------------------------
* Collapse to level x ISCO means     
* ------------------------------
collapse (mean) teleworkable (firstnm) employment wage [aweight = weight], by(level ISCO08_Code)
